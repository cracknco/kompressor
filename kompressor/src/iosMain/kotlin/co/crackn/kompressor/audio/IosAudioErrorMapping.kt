...
// AVFoundation error codes — see AVError.h. Mirrors the video mapper but kept duplicated
// so the audio + video classification tables can drift independently if Apple's audio path
// surfaces a code with different semantics than the video path.
...
private fun build(band: Band, detail: String, cause: Throwable): AudioCompressionError = when (band) {
    Band.UnsupportedSource -> AudioCompressionError.UnsupportedSourceFormat(detail, cause)
    Band.Decoding -> AudioCompressionError.DecodingFailed(detail, cause)
    Band.Encoding -> AudioCompressionError.EncodingFailed(detail, cause)
    Band.Io -> AudioCompressionError.IoFailed(detail, cause)
    Band.Unknown -> AudioCompressionError.Unknown(detail, cause)
}
