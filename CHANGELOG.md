...
* **ci/supply-chain:** CycloneDX 1.5 SBOM as release asset — new `:kompressor:cyclonedxBom` Gradle task (org.cyclonedx.bom 3.2.4, plugin applied at root + `:kompressor`) emits `kompressor/build/reports/bom.json` with the module's full `androidRuntimeClasspath` (~321 components, SHA-1/256/384/512 hashes, VCS external reference, no random `serialNumber` so `syft packages maven-central:...:kompressor:x.y.z` comparisons are byte-level diffable). CI's release pipeline (`.github/workflows/release.yml`) runs the task with `-PsbomVersion=<semver>` (the `version = "0.1.0"` literal beats `ORG_GRADLE_PROJECT_version`, same workaround the SPDX job uses) and uploads `kompressor-x.y.z.sbom.json` alongside the SPDX report. README gains an "SBOM & supply chain" pointer; full verification + optional Dependency-Track ingestion (two strategies) documented in new `docs/supply-chain.md`. Satisfies US EO 14028 / EU CRA supply-chain transparency requirements [CRA-27]
