...
* **test:** iOS >100 MB streaming stress test on device — `IosLargeInputStreamingTest` generates a ~200 MB 1080p 60s H.264 fixture on the fly (no commit), runs it through `IosVideoCompressor.compress`, and asserts process-level peak `phys_footprint` stays ≤ 300 MB (via Mach `task_info(TASK_VM_INFO)`), output is re-probable (AVURLAsset + video track + positive duration), and `onProgress` emits ≥ 5 updates. Runs on AWS Device Farm only (placement in `iosDeviceSmokeTests` bundle) [CRA-11]
