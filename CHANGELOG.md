...
* **test:** Android >100 MB video streaming stress test on device — `LargeVideoInputStreamingTest` (sibling of `IosLargeInputStreamingTests`) generates a ~200 MB 1080p 60s H.264 fixture on-device via `MediaCodec` + `MediaMuxer` (no commit), runs it through `AndroidVideoCompressor.compress`, and asserts process-level peak PSS stays ≤ 300 MB (via `Debug.getPss()` sampled every 50 ms on a background thread), output is re-probable (`MediaExtractor` + video track + positive duration), and `onProgress` emits ≥ 5 updates. Runs on Firebase Test Lab (Pixel 6 API 33) only [CRA-81]
