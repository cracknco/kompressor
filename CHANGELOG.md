...
* **test:** retire hardware-characterization suite per `docs/adr/002-decline-level-3-supply-chain.md` — tests that exercised the *platform's behaviour* (BT.2020 pixel-fidelity ΔE ≤ 2, ITU-R BS.775-3 matrix conformance, AudioToolbox empirical bitrate matrix, inter-process compress via K/N executable + `posix_spawn`, `androidx.startup` initializer race, compile-exhaustive error-taxonomy) rather than the wrapper's contract. Media-compression wrappers cannot generalise device-specific behaviour: an encoded matrix on Pixel 6 / iOS 18 goes stale on the next OS release, so these tests aged toward staleness while adding CI drag. Deleted: `Bs775DownmixMatrixTest` + `Bs775ReferenceFixture` [CRA-13], `AudioToolboxBitrateCharacterizationTest` [CRA-78/CRA-82], `Hdr10PixelFidelityRoundTripTest` + `Hdr10ColorMath` + `Hdr10Mp4Generator` + `GenerateHdr10Fixture` + `scripts/generate-hdr10-fixture.sh` [CRA-6], `ConcurrentCompressInterProcessTest` (Android + iOS) + `InterProcessCompressWorker` + `CompressWorkerMain` + `PosixSpawn.def`/`.h` + `iosSimulatorArm64` `compressWorker` executable target + `KOMPRESSOR_COMPRESS_WORKER_PATH` env wiring [CRA-14, CRA-80], `LargeVideoInputStreamingTest` + `LargeMp4Fixture` + `PeakMemorySampler` (Android side consolidated; iOS `LargeFileCompressionTest` kept) [CRA-11/CRA-81], `KompressorInitializerConcurrencyTest` (tests `androidx.startup`, not us) [CRA-15], `ErrorTaxonomyCompletenessTest` (Detekt `TooGenericExceptionThrown` extension is the remaining static guardrail) [CRA-21], `Surround51RoundTripTest` + `Surround71RoundTripTest` + `IosSurroundAudioTest` (tested the empirical zero-out now reverted), `docs/audio-bitrate-matrix.md`. iOS audio constants (`iosAacMaxBitrate`/`iosAacMinBitrate`/`checkSupportedIosBitrate`) simplified: reverted the mono-44.1 empirical 256 kbps override and the surround zero-out — linear per-channel model only; platform-native rejection surfaces through the existing error-mapping. `docs/audio-downmix.md` trimmed (194 → 37 lines, BS.775 vs Dolby conformance discussion removed). `docs/threading-model.md` trimmed (inter-process section removed) [CRA-38]
